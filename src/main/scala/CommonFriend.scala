...
object CommonFriend {
  ...
  def commonFriend(){
    ...
    /* reduce */
    var reduce = map.map(x=>((x._1._1, x._1._2), x._2.split(",").toSet)).reduceByKey((a,b)=>a.intersect(b)).sortByKey().map(x=>(x._1 + "\t"+x._2.size))
  ...
  def topTenCommonFriend(): Unit ={
    ...
    /* reduce */
    var commonreduce = commonmap.map(x=>((x._1._1, x._1._2), x._2.split(",").toSet)).reduceByKey((a,b)=>a.intersect(b)).sortByKey().map(x=>(x._1._1,x._1._2,x._2.size))
...
//    <Total number of Common Friends><TAB><First Name of User A><TAB><Last Name of User A> <TAB><address of User A><TAB><First Name of User B><TAB><Last Name of User B><TAB><address of User B>
    var friend_reduce = sc.parallelize(friend_map.map(x=>(x._1+"\t"+x._2._1+"\t"+x._2._2+"\t"+x._2._3+"\t"+x._3._1+"\t"+x._3._2+"\t"+x._3._3)).take(10))
  ...
  //.filter(line=>regex.findFirstIn(line(1)).isDefined)
    ...
    var review_map = review_input.map(line=>line.split("::")).map(x=>(x(0),x(1),bussiness_map.get(x(2)).get,x(3))).filter(line=>regex.findFirstIn(line._3).isDefined).map(x=>(x._2,x._4))
    var reduce = review_map.map{case(a,b)=> (a,new StrCollector(b))}.reduceByKey( _ combine _ ).map{case(a,b)=>List(a,b.out).mkString("::")}
  ...
  def avgRrating(): Unit ={
    ...
    var review_reduce = sc.parallelize(review_map.map{case(a,b)=> (a,new AvgCollector(b)) }.reduceByKey( _ combine _ ).map{case(x,y)=>(x,bussiness_map.get(x).get._1,bussiness_map.get(x).get._2,y.avg)}
